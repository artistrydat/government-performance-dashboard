...
export const evaluateProjectCompliance = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        for (const criterion of criteria) {
            const complianceRecord = existingCompliance.find(record => record.criteriaId === criterion._id);
        ...
        const unmetMandatory = criteriaResults.filter(result => mandatoryCriteria.some(c => c._id === result.criteriaId) && result.status === 'not_met');
...
async function evaluateCriteria(criterion, complianceRecord) {
    let score = 0;
    let status = 'not_met';
...
async function validateEvidence(complianceRecord, criterion) {
...
async function performEvaluation(ctx, projectId, standardId, evaluatorId) {
    const now = Date.now();
    // Get project, standard, and criteria data
