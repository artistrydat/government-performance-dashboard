...
export const getProjectRiskStatistics = query({
    ...
    handler: async (ctx, args) => {
        ...
        const totalRiskScore = risks.reduce((sum, risk) => sum + (risk.probability * risk.impact) / 100, 0);
        ...
        const highPriorityRisks = risks.filter(r => r.severity === 'high' || r.severity === 'critical').length;
        return {
            totalRisks,
