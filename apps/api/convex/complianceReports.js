...
export const generateComplianceReportData = query({
    args: {
        ...
        reportType: v.union(v.literal('executive_summary'), v.literal('detailed_breakdown'), v.literal('audit_ready')),
    },
    handler: async (ctx, args) => {
        ...
        for (const project of filteredProjects) {
            ...
            if (projectEvaluations.length > 0) {
                const latestEvaluation = projectEvaluations.reduce((latest, current) => current.evaluatedAt > latest.evaluatedAt ? current : latest);
                totalComplianceScore += latestEvaluation.overallScore;
                evaluatedProjects++;
                const complianceStatus = latestEvaluation.overallScore >= 80
                    ? 'Compliant'
        ...
        const overallCompliance = evaluatedProjects > 0 ? Math.round(totalComplianceScore / evaluatedProjects) : 0;
            ...
            keyFindings: [
                {
                    ...
                    status: overallCompliance >= 80 ? 'good' : overallCompliance >= 60 ? 'warning' : 'critical',
                },
                {
        ...
        const detailedBreakdown = {
            ...
            portfolioBreakdown: portfolios.map(portfolio => {
                ...
                const portfolioEvaluations = evaluations.filter(e => portfolioProjects.some(p => p._id === e.projectId));
                let portfolioScore = 0;
                if (portfolioEvaluations.length > 0) {
                    portfolioScore = Math.round(portfolioEvaluations.reduce((sum, e) => sum + e.overallScore, 0) /
