 ...
 */
export function validateTimelineDates(startDate, endDate) {
 ...
 */
export function validateRiskSeverityConsistency(severity, probability, impact) {
    ...
    if (expectedSeverity !== severity) {
        throw new Error(`Risk severity (${severity}) doesn't match calculated risk score (${riskScore.toFixed(1)}). ` +
 ...
 */
export async function validatePortfolioDeletion(ctx, portfolioId) {
 ...
 */
export async function validateUserDeletion(ctx, userId) {
    const projects = await ctx.db
        .query('projects')
 ...
 */
export async function validateEmailUniqueness(ctx, email, excludeUserId) {
 ...
 */
export async function validateProjectExists(ctx, projectId) {
 ...
 */
export async function validatePortfolioExists(ctx, portfolioId) {
 ...
 */
export async function validateUserExists(ctx, userId) {
 ...
 */
export async function validateRiskExists(ctx, riskId) {
...
export function validateProjectData(projectData) {
    ...
    projectData.timeline.milestones.forEach((milestone, index) => {
        ...
        if (milestone.date < projectData.timeline.startDate ||
 ...
 * @throws Error if any validation fails
 */
export function validateRiskData(riskData) {
